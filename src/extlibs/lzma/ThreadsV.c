...
static WRes _Thread_wait(HANDLE h){
    ...
    rt = thrd_join(self->tid,&join_return);
    if (rt==0)
        self->tid=0;
    return rt;
}
...
WRes Thread_Create(CThread* p,THREAD_FUNC_RET_TYPE (THREAD_FUNC_CALL_TYPE* func)(void *),LPVOID param){
    ...
    rt=thrd_create(&tid,(int(*)(void *))func,param);
    if (rt!=0) return rt;
...
static WRes _Event_wait(HANDLE h) {
    ...
    rt=mtx_lock(&self->mutex);
    if (rt!=0) return rt;
    rt2=SZ_OK;
    while (!self->isState){
        rt2=cnd_wait(&self->cond,&self->mutex);
        if (rt2!=0) break;
    }
    if (!self->isManualReset)
        self->isState=0;
    rt=mtx_unlock(&self->mutex);
    if (rt2!=0) return rt2;
    return rt;
}
...
WRes Event_Set(CEvent* p) {
    ...
    if (rt!=0) return rt;
    self->isState = 1;
    rt=cnd_broadcast(&self->cond);
    rt2=mtx_unlock(&self->mutex);
    if (rt!=0) return rt;
    return rt2;
}
